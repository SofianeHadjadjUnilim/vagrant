...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "meanstack" do |meanstack|
    ...
    meanstack.vm.box = "bento/ubuntu-22.04"
    ...
    meanstack.vm.provision "shell", inline: <<-SHELL
      ...
      ## Ouverture des ports de base pour NodeJs et Angular
      ...
      ## Installation de NodeJS (version 20) et des outils de bases pour son fonctionnement
      ...
      ## Installation de PostgreSQL avec support TLS
      sudo sh -c 'echo "deb http://apt.postgresql.org/pub/repos/apt/ `lsb_release -cs`-pgdg main" >> /etc/apt/sources.list.d/pgdg.list'
      wget -q https://www.postgresql.org/media/keys/ACCC4CF8.asc -O - | sudo apt-key add -
      sudo apt-get -y install postgresql postgresql-contrib
      sudo systemctl start postgresql
      sudo systemctl enable postgresql
      sudo echo "ssl = on" >> /etc/postgresql/$(ls /etc/postgresql/)/main/postgresql.conf

      ## Installation et configuration de MySQL avec support TLS
      ...
      sudo echo "[mysqld]\nssl=on" | sudo tee -a /etc/mysql/mysql.conf.d/mysqld.cnf
